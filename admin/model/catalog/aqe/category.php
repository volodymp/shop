	...
	public function getCategories($data = array()) {
		...
		if (!empty($data['filter_seo'])) {
			if ($this->config->get('aqe_match_anywhere')) {
				...
				foreach ($tokens as $token) {
					$where[] = "(SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = CONCAT('category_id=', cp.category_id) LIMIT 1) LIKE '%" . $this->db->escape($token) . "%'";
				}
			} else {
				$where[] = "(SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = CONCAT('category_id=', cp.category_id) LIMIT 1) LIKE '" . $this->db->escape($data['filter_seo']) . "%'";
			}
		}
