								...
								<th class="<?php echo $column_info[$col]['align']; ?>">
									<select name="filter_<?php echo $col; ?>" class="form-control input-sm search_init fltr <?php echo $col; ?>" data-column="<?php echo $col; ?>">
										...
										<optgroup label="<?php echo $text_choose; ?>">
											<option value="select"<?php echo (!is_null($filters[$col]) && $filters[$col] == "select") ? ' selected' : ''; ?>><?php echo $text_select; ?></option>
											<option value="radio"<?php echo (!is_null($filters[$col]) && $filters[$col] == "radio") ? ' selected' : ''; ?>><?php echo $text_radio; ?></option>
											<option value="checkbox"<?php echo (!is_null($filters[$col]) && $filters[$col] == "checkbox") ? ' selected' : ''; ?>><?php echo $text_checkbox; ?></option>
											<option value="image"<?php echo (!is_null($filters[$col]) && $filters[$col] == "image") ? ' selected' : ''; ?>><?php echo $text_image; ?></option>
										</optgroup>
										<optgroup label="<?php echo $text_input; ?>">
											<option value="text"<?php echo (!is_null($filters[$col]) && $filters[$col] == "text") ? ' selected' : ''; ?>><?php echo $text_text; ?></option>
											<option value="textarea"<?php echo (!is_null($filters[$col]) && $filters[$col] == "textarea") ? ' selected' : ''; ?>><?php echo $text_textarea; ?></option>
										</optgroup>
										<optgroup label="<?php echo $text_file; ?>">
											<option value="file"<?php echo (!is_null($filters[$col]) && $filters[$col] == "file") ? ' selected' : ''; ?>><?php echo $text_file; ?></option>
										</optgroup>
										<optgroup label="<?php echo $text_date; ?>">
											<option value="date"<?php echo (!is_null($filters[$col]) && $filters[$col] == "date") ? ' selected' : ''; ?>><?php echo $text_date; ?></option>
											<option value="time"<?php echo (!is_null($filters[$col]) && $filters[$col] == "time") ? ' selected' : ''; ?>><?php echo $text_time; ?></option>
											<option value="datetime"<?php echo (!is_null($filters[$col]) && $filters[$col] == "datetime") ? ' selected' : ''; ?>><?php echo $text_datetime; ?></option>
										</optgroup>
									</select>
								</th>
									<?php break;
						...
						<tbody>
							...
							<tr>
								...
								<td style="width:1px" class="text-center">
									<input type="checkbox" name="selected[]" value="<?php echo $option['option_id']; ?>"<?php if ($option['selected']) { ?> checked <?php } ?>/>
								</td>
										<?php break;
										case 'action': ?>
								<td class="<?php echo $column_info[$col]['align']; ?> action">
									...
									<?php if ($action['url']) { ?>
										<a href="<?php echo $action['url']; ?>" class="btn btn-default btn-sm <?php echo $action['type']; ?> <?php echo $action['class']; ?>" id="<?php echo $action['action'] . "-" . $option['option_id']; ?>" data-toggle="tooltip" data-container="body" title="<?php echo $action['title']; ?>"><i class="fa fa-<?php echo $action['icon']; ?>"></i><?php echo $action['name']; ?></a>
									<?php } else { ?>
										<button type="button" class="btn <?php echo $action['data'] ? 'btn-warning' : 'btn-default'; ?> btn-sm action <?php echo $action['type']; ?> <?php echo $action['class']; ?>" id="<?php echo $action['action'] . "-" . $option['option_id']; ?>" data-toggle="tooltip" data-container="body" title="<?php echo $action['title']; ?>" data-column="<?php echo $action['action']; ?>"<?php echo ($action['rel']) ? ' data-rel="' . $action['rel'] . '"' : ''; ?>><?php if ($action['icon']) { ?><i class="fa fa-<?php echo $action['icon']; ?>"></i><?php } ?><?php echo $action['name']; ?></button>
									<?php } ?>
									<?php } ?>
									</div>
								</td>
										<?php break;
										default: ?>
								<td class="<?php echo $column_info[$col]['align']; ?><?php echo ($column_info[$col]['qe_status']) ? ' ' . $column_info[$col]['type'] : ''; ?>" id="<?php echo $col . "-" . $option['option_id']; ?>"><?php echo $option[$col]; ?></td>
										<?php break;
									}
								} ?>
							</tr>
...
bull5i.filter=function(){url='index.php?route=catalog/option&token=<?php echo $token; ?>&sort=<?php echo $sort; ?>&order=<?php echo $order; ?>';
...
$('input[name=\'filter_<?php echo $column; ?>\']').autocomplete({source:function(request,response){$.ajax({url:'index.php?route=catalog/option/autocomplete2&token=<?php echo $token; ?>&filter_<?php echo $column; ?>='+encodeURIComponent(request),dataType:'json',success:function(json){response($.unique($.map(json,function(item){
