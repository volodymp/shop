                ...
                <table class="table table-bordered">
                  <thead>
                    <tr>
                      ...
                      <td class="text-right"><span data-original-title="Click Price to Edit" data-toggle="tooltip" title=""><?php echo $column_price; ?></span></td>
		...
		success: function(json) {
			...
			if (json['products'].length) {
				for (i = 0; i < json['products'].length; i++) {
                    ...
                    html += '  <td class="text-right price-editable" data-decimal="' + product['decimal_symbol'] + '" data-product="' + i + '">' + product['price'] + '</td>';
			...
			if (json['totals'].length) {
				for (i in json['totals']) {
					...
					html += '<tr>';
					html += '  ' + (total['code'] == 'shipping' ? '<td class="text-right" colspan="4"><span data-original-title="Click Price to Edit" data-toggle="tooltip" title="">' + total['title'] + ':</span></td>' : '<td class="text-right" colspan="4">' + total['title'] + ':</td>');
					html += '  ' + (total['code'] == 'shipping' ? '<td class="text-right price-editable" data-decimal="' + total['decimal_symbol'] + '" data-product="' + total['code'] + '">' + total['text'] + '</td>' : '<td class="text-right">' + total['text'] + '</td>');
					html += '</tr>';
				}
			}
...
$('#content').on('click', '.price-editable', function() {
	...
	var price = Number(productPrices[row].replace(regex,"").replace(decimal_symbol,'.')).toFixed(<?php echo $currency_decimal_place; ?>);
...
$('#content').on('input', '.price-override', function() {
	...
	if (keyCode != 27 && +rowPrice != +this.value && !locked) {
		<?php if (version_compare(VERSION, '2.3.0.0', '>=')) { ?>
			var override_url = '<?php echo $catalog; ?>index.php?route=api/cart/price_override&token=' + token + '&store_id=' + $('select[name=\'store_id\'] option:selected').val();
		<?php } else if (version_compare(VERSION, '2.2.0.0', '>=')) { ?>
			var override_url = '<?php echo $store_url; ?>index.php?route=api/cart/price_override&token=' + token + '&store_id=' + $('select[name=\'store_id\'] option:selected').val();
		<?php } else if (version_compare(VERSION, '2.0.3.1', '>')) { ?>
			var override_url = $('select[name=\'store\'] option:selected').val() + 'index.php?route=api/cart/price_override&token=' + token;
		<?php } else { ?>
			var override_url = 'index.php?route=sale/order/api&token=<?php echo $token; ?>&api=api/cart/price_override&store_id=' + $('select[name=\'store_id\'] option:selected').val();
		<?php } ?>
		locked = true;
		$("html").addClass("wait");
		$.ajax({
