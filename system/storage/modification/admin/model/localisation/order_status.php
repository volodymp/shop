...
class ModelLocalisationOrderStatus extends Model {
	...
	public function addOrderStatus($data) {
		foreach ($data['order_status'] as $language_id => $value) {
			if (isset($order_status_id)) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_status SET order_status_id = '" . (int)$order_status_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', order_status_subject = '" . $this->db->escape($value['order_status_subject']) . "', os_email_template = '" . $this->db->escape($value['os_email_template']) . "'");
			} else {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_status SET language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', order_status_subject = '" . $this->db->escape($value['order_status_subject']) . "', os_email_template = '" . $this->db->escape($value['os_email_template']) . "'");
	...
	public function getOrderStatusDescriptions($order_status_id) {
		...
		foreach ($query->rows as $result) {
			$order_status_data[$result['language_id']] = array('name' => $result['name'], 'order_status_subject' => $result['order_status_subject'], 'os_email_template' => $result['os_email_template']);
		}
